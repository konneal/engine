...
- The vector adapter (`ingest/vector_adapter.py`, contract in `docs/vector-adapter.md`) is the ONLY door from producer chunks to any Vectorize index: pydantic wire schema (registry, size caps, anchor sanity) + target gating (lane corpora structurally cannot enter production — 2026-09-02 incident). Every new producer/upsert path goes through `normalize_chunk(...).upsert(..., target=…)`, never a hand-rolled metadata dict. Raw-jsonl consumers (cli upsert, restore, replay) project through `wire_meta` — Vectorize metadata is scalar-or-string[] law, and rich producer payloads (metadata.table) are derivation records, never wire state (the 2026-09-11 40017 incident: rejected batches surfacing as opaque binding 502s). The canonical chunk set is declared ONCE in `ingest/config.py` (`CANONICAL_CHUNK_SOURCES`) and consumed by every tool. `/admin/enrich` default mode writes to the production index; callers that only want contexts pass `mode:"context"`.
