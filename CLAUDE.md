...
- Unit suites (plain node, no network): `node --test --experimental-strip-types tests/refs.test.ts tests/lexical.test.ts tests/anchors.test.ts tests/structural.test.ts tests/verdict.test.ts tests/chunkmeta.test.ts tests/pipeline.test.ts` — same list as `scripts/deploy.sh`; includes the stage-composition test and the ChunkMeta↔pydantic contract test
- Serving architecture: retrieval is a stage registry (`workers/worker_public/src/stages/` — contracts in `docs/spec-pipeline.md`; `retrieve()` is composition only; add a mechanism = stage file + registry entry); HTTP is a route table (`ROUTES` in `src/index.ts` via `src/lib/router.ts` — inventory in `docs/spec-api.md`); every steering number lives in `THRESHOLDS` (`src/config.ts`); the chunk wire type is `workers/shared/chunk.ts`, kept in sync with `ingest/vector_adapter.py` by the contract test; grading vocabulary shared by all runners lives in `tests/grade.mjs`
